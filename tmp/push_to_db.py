...
# 3. DB 푸시
print(f"🚀 {len(df)}명의 분석 데이터를 churn_prediction 테이블로 전송 중...")
try:
    with engine.begin() as conn:
        conn.execute(text("DELETE FROM churn_prediction"))
        df.to_sql('churn_prediction', conn, if_exists='append', index=False)
